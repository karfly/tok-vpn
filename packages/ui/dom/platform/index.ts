...
export function isIOS(): boolean {
  return (
    ...
    (isApplePlatform() && navigator.maxTouchPoints > 1)
  );
}
...
export function isSafari(element: Node): boolean {
  const documentRef = element.ownerDocument;
  const windowRef =
    documentRef &&
    (documentRef.defaultView as unknown as Window & {
      safari?: any;
    });
  ...
  const isMacOsSafari =
    ...
    !!windowRef.safari &&
    !!windowRef.safari.pushNotification &&
